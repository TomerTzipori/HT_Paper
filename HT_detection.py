...
def calc_cv(input_list, bool_func):
    ...
    for cofactor in tqdm(range(num_inputs)):
        ...
        while i < max_num_rows:
            ...
            result_0 = bool_func.evaluate(input_dict)

            # cofactor equals 1
            bin_list[cofactor] = 1
            hal_list[cofactor] = hal_py.BooleanFunction.Value(bin_list[cofactor])
            input_dict[input_list[cofactor]] = hal_list[cofactor]
            result_1 = bool_func.evaluate(input_dict)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sort_gates', action='store_true', required=False, help='Traverses the gates in the analysis according to their input size (from bottom to top)')
    ...
    # iterates through the filtered gates (first iteration)
    print("Performing First Iteration...")
    for gate in tqdm(filtered_gates):
        # gets output wires
        ...
        dec = hal_py.SubgraphNetlistDecorator(netlist=netlist)
        sub_bool_func = dec.get_subgraph_function(subgraph_gates=sub_gates, subgraph_output=fanout_net[0])
        ...
        df = pd.concat([df, pd.DataFrame({'GATE_NAME': gate.get_name(), 'GATE_TYPE': gate.get_type().get_name(),
                                          ...
                                          'CONTROL_VECTOR': "", 'MEAN': 0.0,
                                          'MEDIAN': 0.0, 'SIZE': len(variables)}, index=[len(df)])])
        df.to_csv(csv_path)

    # check if --sorted option is selected. if so, changes the order we traverse the gates
    if args.sort_gates:
        sorted_gates = df.sort_values(by='SIZE', ascending=True)['GATE_NAME'].values  # warning: this column only contains names, not gates
    else:
        sorted_gates = df['GATE_NAME'].values  # warning: this column only contains names, not gates

    # we are ditching "for gate in filtered_gates" for something based on the df column

    # iterates through sorted_gates (second iteration)
    print("Performing Second Iteration...")
    for gate_name in sorted_gates:
        # gets cur row from the og dataframe
        cur_row = df.loc[df['GATE_NAME'] == gate_name]

        # prints the name and the type of the current gate
        print(f'CURRENT GATE: {gate_name} TYPE: {cur_row.iloc[0]["GATE_TYPE"]}')
        print("------------------------------------------------------------------------------------")

        # obtaining the bool func and the variables
        sub_bool_func, variables = bool_func_dict[gate_name]

        control_vector = []
        mean_cv = 0
        median_cv = 0
        if len(variables):
            # calculating the control value vector
            control_vector = calc_cv(variables, sub_bool_func)
            ...
            df.loc[df['GATE_NAME'] == gate_name, 'CONTROL_VECTOR'] = str(control_vector)
            df.loc[df['GATE_NAME'] == gate_name, 'MEAN'] = mean_cv
            df.loc[df['GATE_NAME'] == gate_name, 'MEDIAN'] = median_cv
            df.to_csv(csv_path)
